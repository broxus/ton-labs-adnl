use crate::{
    common::{
        deserialize, serialize_inplace, AdnlHandshake, AdnlPeers, AdnlPingSubscriber, AdnlStream,
        AdnlStreamCrypto, KeyId, KeyOption, KeyOptionJson, Query, Subscriber, Timeouts, TARGET,
    },
    dump, from_slice,
};
use futures::prelude::*;
...
impl AdnlServerThread {
    ...
    fn parse_init_packet(
        ...
        let local_key = AdnlHandshake::parse_packet(key, buf, Some(160))?
            .ok_or_else(|| error!("Unknown ADNL server key, cannot decrypt"))?;
        let other_key = KeyOption::from_type_and_public_key(KeyOption::KEY_ED25519, &other_key)
            .id()
            .clone();
        dump!(trace, TARGET, "Nonce", &buf[..160]);
        let ret = AdnlStreamCrypto::with_nonce_as_server(arrayref::array_mut_ref!(buf, 0, 160));
        buf.drain(0..160);
